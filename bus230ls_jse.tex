...
\begin{titlepage}
\begin{singlespace}
\title{\vspace*{-1in}Developing Students' Thought Processes For Choosing Appropriate Statistical Methods}
\date{\today}
\author{
}
...
\abstract{Introductory statistics classes typically emphasize computation and implementation procedures for a variety of statistical tests.  While these skills are necessary, students may struggle in subsequent research methods courses when expected to select statistical tests that are appropriate to answer a research question.  This task requires an understanding of how statistical methods are related to one another; students must develop an advanced organization of knowledge.  In this paper we report on a \textit{lesson study} we conducted to better understand students' thought processes.  A lesson study is a teaching improvement activity in which instructors jointly develop a lesson and observe and record student learning, making the learning process visible.  This paper makes two significant contributions which can help instructors of research methods courses improve student learning.  First, we share our thought process map for selecting a statistical test, report on the impact it had for our students, and offer suggestions for improving this part of the lesson.  Secondly, we describe the thought processes that students go through, both before and after being exposed to the thought process map, and we describe sources of confusion revealed through the lesson study process.  Students struggled with knowing when to apply an independent-samples test versus a paired-samples test; knowing how the question of scale of measurement informs the statistical method; recognizing and defining variables; and understanding statistical versus colloquial language. 
}\newline  

\noindent \textit{Keywords}: Lesson study, introductory statistics, research methods. \\
%\noindent \textit{JEL classification}: A22.
\end{singlespace}
\end{titlepage}
\newpage
